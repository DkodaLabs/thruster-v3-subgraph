...
import {FACTORY_ADDRESS, ONE_BD, ONE_BI, ZERO_BD, ZERO_BI} from '../utils/constants'
import {
  findEthPerToken,
  getEthPriceInUSD,
  getTrackedAmountUSD,
...
export function handleSwap(event: SwapEvent): void {
  ...
  pool.tick = BigInt.fromI32(changetype<i32>(event.params.tick))
  pool.sqrtPrice = event.params.sqrtPriceX96
  pool.totalValueLockedToken0 = pool.totalValueLockedToken0.plus(amount0)
  pool.totalValueLockedToken1 = pool.totalValueLockedToken1.plus(amount1)
  ...
  swap.tick = BigInt.fromI32(changetype<i32>(event.params.tick))
